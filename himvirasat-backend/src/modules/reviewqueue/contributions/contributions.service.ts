...
import {
  ...
  reviewQueueRepository,
  ExtendedContributionFilters,
} from "../reviewqueue.repository.js";
import { logger } from "../../../utils/logger.js";
import type { HistoryEventType } from "@himvirasat/shared";
...
export class ContributionsService {
  ...
  async create(ctx: SecurityContext, payload: Record<string, unknown>) {
  ...
  async fetch(ctx: SecurityContext, filters: ContributionFilters) {
    const { actor } = ctx;
    const isAdmin =
      actor.role === "super_admin" || actor.role === "language_head";

    const effectiveFilters: ExtendedContributionFilters = {
      ...filters,
      ...(!isAdmin ? { dialect_names: actor.dialects || [] } : {}),
    };
    ...
    return data.map((item) => formatContribution(item));
  }

  async fetchByIdFromContributions(_ctx: SecurityContext, id: string) {
  ...
  async delete(ctx: SecurityContext, id: string) {
