...
export const createSubmission: RequestHandler = async (req, res): Promise<void> => {
    ...
    const validationResult = CreateSubmissionSchema.safeParse({
        ...
        part_of_speech_id: req.body.part_of_speech_id ? Number(req.body.part_of_speech_id) : null,
    });
    if (!validationResult.success) {
        const errorDetails = validationResult.error.issues.map((i) => i.message).join(", ");
        res.status(400).json({ success: false, error: `Validation error: ${errorDetails}` });
        return;
    }
    try {
        const contribution = await service.createSubmission(contributor_id, validationResult.data);
        res.status(201).json({ success: true, message: "Vocabulary entry submitted successfully.", data: contribution });
    } catch (error: any) {
