...
export async function getLanguageExperts(_req: Request, res: Response) {
  try {
    ...
    console.error(error);
    return res.status(500).json({ success: false, message: "Failed to fetch language experts" });
  }
}
...
export async function createLanguageExpert(req: Request, res: Response) {
  try {
    ...
    console.error(error);
    return res.status(500).json({ success: false, message: "Failed to create language expert" });
  }
}
...
export async function deleteLanguageExpert(req: Request, res: Response) {
  try {
    ...
    return res.status(200).json({ success: true, message: "Language expert deactivated", deleted_id: id });
  } catch (error) {
    console.error(error);
...
export async function getLanguageHeads(_req: Request, res: Response) {
  try {
    ...
    console.error(error);
    return res.status(500).json({ success: false, message: "Failed to fetch language heads" });
  }
}
...
export async function createLanguageHead(req: Request, res: Response) {
  try {
    ...
    console.error(error);
    return res.status(500).json({ success: false, message: "Failed to create language head" });
  }
}
...
export async function deleteLanguageHead(req: Request, res: Response) {
  try {
    ...
    return res.status(200).json({ success: true, message: "Language head deactivated", deleted_id: id });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ success: false, message: "Internal server error" });
  }
...
export async function updateExpertDialects(req: Request, res: Response) {
  try {
    ...
    if (!id || !Array.isArray(dialects)) {
      return res.status(400).json({ success: false, message: "User ID and dialects array are required" });
    }
    const result = await service.updateExpertDialects(id, dialects);
    if (!result.success) {
      return res.status(result.statusCode ?? 500).json({ success: false, message: result.message });
    }
    return res.status(200).json({ success: true, message: "Expert dialects updated", expert: result.data });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ success: false, message: "Failed to update dialects" });
  }
}
...
export async function updateHeadDialects(req: Request, res: Response) {
  try {
    ...
    return res.status(200).json({ success: true, message: "Head dialects updated", head: result.data });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ success: false, message: "Failed to update managed dialects" });
  }
}